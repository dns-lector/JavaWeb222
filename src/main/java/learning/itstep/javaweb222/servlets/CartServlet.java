...
public class CartServlet extends HttpServlet {
    ...
    private void doLink(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        cartId = cartId.substring(1);  // прибираємо перший "/"
    ...
    protected void doPatch(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if(inc == null || inc.isBlank()) {
            this.restResponse.setStatus(RestStatus.status400);
            this.restResponse.setData("Missing query parameter: inc");
            return;
        }
        int d;
        try { d = Integer.parseInt(inc); }
            ...
            this.restResponse.setStatus(RestStatus.status400);
            this.restResponse.setData("Query parameter inc must be decimal number");
            return;
        }
        if(d == 0) {
            this.restResponse.setStatus(RestStatus.status400);
            this.restResponse.setData("Query parameter inc is zero");
            return;
        }
        try {
